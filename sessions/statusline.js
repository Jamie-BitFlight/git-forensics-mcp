  ...
  try {
    ...
    for (const candidate of allFiles) {
      try {
        ...
        const candidateLast = JSON.parse(
...
function main() {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (lineData.message?.usage) {
            ...
            if (
              timestamp &&
              (!mostRecentTimestamp || timestamp > mostRecentTimestamp)
            ) {
  ...
  const formattedTokens = contextLength
    ? `${Math.floor(contextLength / 1000)}k`
    : "17k";
  const formattedLimit = contextLimit
    ? `${Math.floor(contextLimit / 1000)}k`
    : "160k";

...
class TriggerPhrases {
  ...
  addPhrase(category, phrase) {
    ...
    if (!list || !Array.isArray(list))
      throw new Error(`Unknown trigger category: ${category}`);
  ...
  listPhrases(category = null) {
    if (category) {
      if (typeof category === "string")
        category = this._coaxPhraseType(category);
      const list = this[category];
      if (!list || !Array.isArray(list))
        throw new Error(`Unknown trigger category: ${category}`);
...
class EnabledFeatures {
  constructor(data = {}) {
    ...
    this.task_detection =
      data.task_detection !== undefined ? data.task_detection : true;
    this.auto_ultrathink =
      data.auto_ultrathink !== undefined ? data.auto_ultrathink : true;
    ...
    if (iconStyleValue === undefined || iconStyleValue === null) {
      ...
      if (oldUseNerdFonts !== undefined && oldUseNerdFonts !== null) {
        ...
        iconStyleValue = oldUseNerdFonts
...
class TaskState {
  ...
  static loadTask(options = {}) {
    ...
    if (!fs.existsSync(fullPath))
      throw new Error(`Task file ${fullPath} does not exist.`);

    const content = fs.readFileSync(fullPath, "utf8");
    const fmStart = content.indexOf("---");
    if (fmStart !== 0)
      throw new StateError(`Task file ${fullPath} missing frontmatter.`);

    const fmEnd = content.indexOf("---", fmStart + 3);
    if (fmEnd === -1)
      throw new StateError(`Task file ${fullPath} missing frontmatter end.`);
...
class SessionsTodos {
  ...
  stashActive(force = true) {
    ...
    throw new StashOccupiedError(
      "Stash already occupied. Use force=true to overwrite.",
    );
  }
...
class SessionsState {
  ...
  static fromDict(data) {
    ...
    const activeTodos = (todosData.active || []).map((t) =>
      SessionsState._coerceTodo(t),
    );
    const stashedTodos = (todosData.stashed || []).map((t) =>
      SessionsState._coerceTodo(t),
    );
      ...
      try {
        ...
        if (fs.existsSync(lockInfoFile)) {
          ...
          if (now - lockTime > staleTimeout) {
            console.error(
              `Removing stale lock (age: ${(now - lockTime).toFixed(1)}s)`,
            );
            try {
    ...
    try {
      ...
      if (Date.now() / 1000 - start > timeout) {
        ...
        try {
          ...
          throw new Error(
            `Could not acquire lock ${LOCK_DIR} even after force removal`,
          );
        }
      }
      sleepSync(pollMs);
...
function loadConfig() {
  ...
  try {
    ...
    if (
      data.features &&
      "use_nerd_fonts" in data.features &&
      !("icon_style" in data.features)
    ) {
      needsMigration = true;
    }

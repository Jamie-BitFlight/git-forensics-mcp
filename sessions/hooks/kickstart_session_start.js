...
const SUBAGENTS_MODE_SEQUENCE = ["01-agents-only.md"];
///-///
  ...
  console.log(
    JSON.stringify({
      hookSpecificOutput: {
        ...
        additionalContext:
          "ERROR: kickstart_session_start hook fired but no kickstart metadata found. This is an installer bug.",
      },
    }),
  );
  process.exit(1);
  ...
  console.log(
    JSON.stringify({
      hookSpecificOutput: {
        hookEventName: "SessionStart",
        additionalContext:
          "ERROR: kickstart metadata exists but no mode specified. This is an installer bug.",
      },
    }),
  );
  process.exit(1);
...
const sessionsCmd = isWindows
  ? "sessions/bin/sessions.bat"
  : "sessions/bin/sessions";

const beginCmd =
  mode === "subagents"
    ? `${sessionsCmd} kickstart subagents`
    : `${sessionsCmd} kickstart full`;

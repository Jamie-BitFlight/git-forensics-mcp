...
function beginKickstart(mode, jsonOutput = false) {
  ...
  const sequence =
    mode === "subagents" ? SUBAGENTS_MODE_SEQUENCE : FULL_MODE_SEQUENCE;
  editState((s) => {
...
function completeKickstart(jsonOutput = false) {
      ...
      if (typeof cmd === "string" && cmd.includes("kickstart_session_start")) {
        if (cmd.includes("kickstart_session_start.js")) {
          hook.command = cmd.replace(
            "kickstart_session_start.js",
            "session_start.js",
          );
          updatedSettings = true;
        } else if (cmd.includes("kickstart_session_start.py")) {
          hook.command = cmd.replace(
            "kickstart_session_start.py",
            "session_start.py",
          );
          updatedSettings = true;
        }
  ...
  if (hooksRoot && typeof hooksRoot === "object")
    hooksRoot.SessionStart = newCfgs;
  if (updatedSettings) {
    try {
  ...
  const successMessage =
    "Kickstart complete! Cleanup finished and SessionStart restored.";
